...
namespace RetakesFormatter
{
    ...
    {
        public void FormatHeader(ExcelWorksheet BaseSheet, ExcelWorksheet ReformattedSheet)
        {
        ...
        public void FormatInsertedLines(ExcelWorksheet sheet)
        {
            ...
            for (int row = 13; row <= end.Row; row++)
            {
                ...
                AutofitRows.DoAutoFitForReformatted(sheet, row);
        ...
        /// Because Epplus doesn't support partial styling for existing data and only supports formatting new lines I had to:
        /// 1. get the text before and after the string I want to format
        /// 2. Insert each section (before text, the targetd text, after text)
        /// 3. Format them manually
        /// </summary>
